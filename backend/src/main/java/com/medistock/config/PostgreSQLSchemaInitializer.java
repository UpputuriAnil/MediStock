...
public class PostgreSQLSchemaInitializer {
    ...
    public void initializeTables() {
        ...
        try {
            ...
            jdbcTemplate.execute("CREATE TABLE IF NOT EXISTS expiry_tracking (" +
                    "id BIGSERIAL PRIMARY KEY, " +
                    "medicine_id BIGINT, " +
                    "inventory_id BIGINT, " +
                    "medicine_name VARCHAR(200), " +
            ...
            jdbcTemplate.execute("CREATE TABLE IF NOT EXISTS stock_logs (" +
                    "id BIGSERIAL PRIMARY KEY, " +
                    "medicine_id BIGINT, " +
                    "medicine_name VARCHAR(200), " +
                    "batch_number VARCHAR(100), " +
            ...
            jdbcTemplate.execute("CREATE TABLE IF NOT EXISTS email_verification_tokens (" +
                    "id BIGSERIAL PRIMARY KEY, " +
                    "token VARCHAR(255) NOT NULL UNIQUE, " +
                    "user_id BIGINT NOT NULL UNIQUE, " +
                    "expiry_date TIMESTAMP NOT NULL, " +
            ...
            log.info("PostgreSQLSchemaInitializer: All 17 database tables created and verified successfully!");
        } catch (Exception e) {
            log.error("PostgreSQLSchemaInitializer notice: {}", e.getMessage());
        }
    }
}
